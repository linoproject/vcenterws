 ...
 */
connectDB = function()  {
	const dbConnection_vcenterws_db = mongoose.createConnection('mongodb://' + properties.vcenterws_db_dbUrl, function(err){
		if(err) {
			logger.error(err);
			setTimeout(function() {
		...
		{
		    exports.connection = dbConnection_vcenterws_db;
			logger.info("MongoDB connected at: " + properties.vcenterws_db_dbUrl);
